...
- `list:words:min` a random number of words generated as elastic search array field (multi valued field) from a given list of `-` seperated words. You can specify a minimum number of words, e.g. to avoid empty lists. This is an optional parameter that defaults to 0. Example: list:first-second-third:! will generate arrays of 1 or 3 items into the field.
